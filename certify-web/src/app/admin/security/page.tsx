...
export default function AdminSecurityPage() {
  ...
  async function turnOff() {
    const c = prompt(t("admin.security.disablePrompt"));
    if (!c) return;
    setBusy(true); setError(null); setNotice(null);
    try {
      ...
      setPhase("idle");
      setBackupCodes(null);
      setNotice(t("admin.security.disabledNotice"));
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setBusy(false);
  ...
  return (
    <main className="mx-auto max-w-2xl p-6">
      <h1 className="flex items-center gap-2 font-display text-2xl font-black text-ink">
        <IconLock className="h-6 w-6 text-brand-600" /> {t("admin.security.title")}
      </h1>
      ...
      <section className="card mt-6 p-6">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-ink">{t("admin.security.mfaTitle")}</p>
              <p className="text-xs font-bold">
                {enabled === null ? "…" : enabled
                  ? <span className="text-green-700">{t("admin.security.enabled")}</span>
                  : <span className="text-ink-muted">{t("admin.security.disabled")}</span>}
              </p>
            </div>
          </div>
          {enabled
            ? <button onClick={turnOff} disabled={busy} className="rounded-xl px-4 py-2.5 text-sm font-bold text-red-600 transition hover:bg-red-50 disabled:opacity-60">{t("admin.security.disable")}</button>
            : phase === "idle" && <button onClick={startSetup} disabled={busy} className="btn-primary">{busy ? "…" : t("admin.security.enable")}</button>}
        </div>

        {/* خطوة الإعداد: QR + تأكيد */}
        {phase === "setup" && qr && (
          <form onSubmit={confirmEnable} className="mt-6 border-t pt-6">
            <p className="text-sm font-bold text-ink">{t("admin.security.step1")}</p>
            <div className="mt-3 flex flex-col items-center gap-3">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={qr.qr_data_url} alt={t("admin.security.qrAlt")} className="h-44 w-44 rounded-lg ring-1 ring-line" />
              <p className="text-xs text-ink-muted">{t("admin.security.orManual")}</p>
              <code dir="ltr" className="rounded-lg bg-surface-2 px-3 py-1.5 text-sm font-bold tracking-wider text-ink">{qr.secret}</code>
            </div>
            <p className="mt-5 text-sm font-bold text-ink">{t("admin.security.step2")}</p>
            <input
            ...
            <div className="mt-4 flex gap-3">
              <button type="submit" disabled={busy || code.length < 6} className="btn-primary disabled:opacity-60">
                {busy ? t("admin.security.enabling") : t("admin.security.confirmEnable")}
              </button>
              <button type="button" onClick={() => { setPhase("idle"); setQr(null); setCode(""); }} className="btn-ghost">{t("admin.security.cancel")}</button>
            </div>
          </form>
        )}
        ...
        {phase === "done" && backupCodes && (
          <div className="mt-6 border-t pt-6">
            <p className="flex items-center gap-1.5 text-sm font-bold text-green-700">
              <IconCheck className="h-4 w-4" /> {t("admin.security.enabledNotice")}
            </p>
            <div className="mt-4 rounded-xl bg-amber-50 p-4 ring-1 ring-amber-100">
              <p className="text-sm font-bold text-amber-800">{t("admin.security.saveBackup")}</p>
              ...
              <p className="mt-3 text-xs text-amber-700">{t("admin.security.backupNote")}</p>
            </div>
          </div>
        )}
      </section>
