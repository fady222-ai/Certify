...
- **التحوّلات:** ردّ الأدمن → `answered`؛ ردّ العميل على تذكرة `open`/`answered` → `open`.
  **الإغلاق صلاحية الأدمن فقط** (عبر `adminSetStatus` → `closed`؛ أُزيل إغلاق المستخدم
  وزره ومساره `/close`). **الإغلاق نهائي:** الردّ على تذكرة `closed` يُرفَض بـ409 (مستخدم
  وزائر) — يجب فتح تذكرة جديدة. الأدمن وحده يستطيع الردّ/تغيير حالة المغلقة.
