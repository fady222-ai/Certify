...
**نموذج الانضمام (قرار المالك): المالك يُنشئ الحساب مباشرةً** (اسم/بريد/كلمة مرور مؤقتة + دور) —
بلا بريد/روابط دعوة؛ العضو يسجّل الدخول فوراً (`emailVerified=true`). عضو الفريق **لا يملك أكاديمية
خاصة** — له صفّ `OrganizationMember` فقط.
- **حلّ المنظمة أصبح membership-aware:** `authService.primaryOrg` يُرجِع المنظمة المملوكة أولاً
  (توافق خلفي) وإلا منظمة العضوية؛ و`orgRole(org,userId)` يُرجِع `owner`/`admin`/`member`.
  `middleware/auth.js` يضبط `req.membershipRole`. (المالك له صفّ عضوية `owner` يُنشأ عند التسجيل.)
- **الأدوار:** `member` (إصدار/إدارة الشهادات والقوالب) · `admin` (+ إدارة الأعضاء) · `owner`
  (+ الفوترة والإعدادات). **حُصِّنت مُتغيّرات الفوترة لـ`requireOwner`** (`/billing/checkout|plan|cancel`)
  فلا يغيّر عضوٌ الباقة؛ تعديل المنظمة/العلامة محصور بالمالك أصلاً (`ownerId`).
- **المتحكّم** `controllers/memberController.js`: `GET /api/members` (قائمة + `can_manage` + `limit`) ·
  `POST /api/members` (owner/admin؛ يفرض الحدّ، يرفض البريد المكرّر 409) · `PATCH /api/members/:id`
...
- **الإيميلات بالإنجليزية دائماً (الخلفية):** **قرار المنتج: كل البريد الصادر بالإنجليزية** —
  تفعيل الحساب (OTP)، استعادة كلمة المرور، الفوترة، الشهادة، الدعم (شاملة إشعارات الأدمن وإيصالات
  الزائر) — بصرف النظر عن `User.locale`. المصدر الموحَّد `services/email/emailI18n.js`: `effLocale()`
  تُرجِع `"en"` و`isEn()` تُرجِع `true` دائماً (نقطة تبديل واحدة لو لزم تغيير القرار). **لا متغيّر
  بيئة — السلوك مدمج.** القوالب تبقى ثنائية اللغة (فرع عربي محفوظ غير مستخدَم) فلا يلزم إعادة كتابتها
  إن غُيِّر القرار لاحقاً. (يبقى التقاط `User.locale` عند التسجيل وتحديثه عبر `POST /api/auth/locale`
  لأغراض الواجهة، لكنه لا يؤثّر على لغة البريد.) اختبار `test/email-i18n.test.js` يثبّت أن كل بريد
  إنجليزي حتى لمستلِم عربي.
...
- **تحقق OTP بالبريد**: التسجيل لا يُصدر JWT فوراً — يُرسل رمز 6 أرقام (صلاحية
  ...
  5 محاولات خاطئة → 429).
- **الدخول بالبريد فقط**: `login` يستقبل `identifier` ويبحث به **بالبريد حصراً**
  (`findUserByIdentifier`). أُزيل الدخول باسم الأكاديمية (كان يوسّع سطح التخمين لأن اسم
  الأكاديمية عام) — اسم أكاديمية لا يُطابق أيّ حساب ويُرجِع 401 كأي معرّف مجهول. الواجهة:
  حقل الدخول `type="email"` بنصّ «البريد الإلكتروني». لا حقل `username` منفصل.
...
- **اختبارات** (`test/auth.test.js`، تُشغَّل بـ `npm test`): تثبّت دفاعات التخمين
  ...
  رفض الدخول باسم الأكاديمية (بريد فقط)، بريد غير مفعّل → 403 مع إعادة إرسال OTP).
...
- **مكتبة القوالب الجاهزة:** `certify-api/src/templates/presetTemplates.js` يولّد قوالب **عامة**
  ...
  داكن نيلي، لوحة زمردية، إطار متدرّج + بطاقة بيضاء) **و4 احترافية** (17–20: آرت ديكو، أمان
  guilloche، شريط سفلي، عمودي عصري) **وقالب موجّه** (21) ليصبح المجموع 21. **بعد النشر شغّل
  `node prisma/seed.js` لإضافتها.** (خلفية الصورة الفوتوغرافية تبقى لكل منظمة ترفعها بنفسها.)
- **القالب الموجّه + تخصيص المحتوى:** القالب 21 (الشعار أعلى → «شهادة» → جملة الأكاديمية →
  فوتر: QR وسط + التاريخ يميناً وتوقيع الأكاديمية يساراً) يضمّ عنصرَي نصّ موسومين `role:"title"`
  و`role:"body"`. **تحرير المحتوى (نوع الشهادة + الجملة) في مُخصِّص القالب** (`dashboard/templates/
  customize`) لا في المحرّر العام: قائمة **نوع الشهادة** (`CERT_TYPES`) تكتشف العنوان (role أو نصّ
  يبدأ بـ«شهادة») فتجعله «شهادة حضور/تقدير/…»، و**نصّ الشهادة** (`textarea`) يظهر إن وُجد body.
  المنطق النقي في `lib/customize.ts` (`readCertType`/`readBody`/`hasBody`/`applyContent`) ويُطبَّق
  في `themedDesign` بعد `applyTheme`. (المحرّر العام «من الصفر» يبقى أدواته الحرّة دون لوحة محتوى.)
  `canvas.ts` يحفظ `role` عبر دورة الحفظ (مغطّى بـ`canvas.test.ts`).
...
- **مصدر حقائق الباقات موحّد (الخلفية = المرجع):** الكتالوج المرجعي للباقات
  ...
  باقة بالكتالوج المرجعي ويُفشل CI عند أي انحراف — فلا يمكن تغيير سعر/حصة/مقاعد في جهة
  دون الأخرى. الحصة ومقاعد الفريق تُكتبان رقماً واحداً (`certsPerMonth`/`teamMembers`) ويُشتقّ
...
- **تحصين الإصدار الجماعي (`batchController.js`):** (1) تحليل CSV بـ`bom:true` فلا تنكسر ملفات
  ...
  واتساب. (5) **قالب Excel جاهز للتنزيل:** `GET /api/batches/template` (`downloadTemplate`، مُسجَّل قبل
  `/:id`) يبني `.xlsx` عبر ExcelJS بالأعمدة المقبولة (name/email/course_name/phone) + صفّ مثال + رأس
  منسّق، تنزّله الأكاديمية وتملؤه وتعيد رفعه. زر «تحميل قالب Excel» في صفحة الرفع. اختبار round-trip
  (التوليد→`parseFile`) في `test/batch.test.js`. مغطّى بـ`test/batch.test.js`.
- **تسريع الإصدار الجماعي (تزامن آمن + حجز حصة):** بدل المعالجة صفّاً صفّاً، `processBatchAsync`
  يحجز الحصة **مرّة واحدة مقدّماً** عبر `reserveQuota(org, actingUserId, n)` (المتبقّي = حدّ الباقة −
  استخدام الشهر، **و** حدّ العضو − ما أصدره) فيُصدر فقط `allowed` صفّاً والباقي يُعدّ فاشلاً (تجاوز
  حصة)، ثم يعالج بـ**مجموعة عمّال متوازية** (`CONCURRENCY=4`، كل صفّ صفحة Chrome على متصفّح مشترك) →
  أسرع ~4× لـ500 شهادة دون خطر تجاوز الحدّ. لأن الحصة محجوزة مسبقاً، الإصدار داخل العمّال يمرّر
  `skipLimits:true` لـ`issueCertificate` (يتخطّى فحصَي الباقة/العضو المتكرّرين والقابلين للتسابق)؛
  و`ensureUsageRow` يُنشئ صفّ الاستخدام مسبقاً فتبقى الزيادات الذرّية بلا تسابق على الإنشاء.
  `createBatch` يفشل مبكراً بـ402 إن كان `allowed===0`. (`skipLimits` حصري لمسار الدفعة بعد الحجز —
  لا يُكشف لمسار فردي.) مغطّى بـ`certificate-issuer.test.js` (reserveQuota) و`batch.test.js` (تجاوز/skipLimits).
...
- **الحذف النهائي** (`DELETE /api/certificates/:id` → `deleteCertificate`): يحذف سجلّ
  الشهادة كلياً (أحداثه تتعاقب بـ`onDelete:Cascade`) ويحذف ملف الـPDF من القرص. **يُنقِص
  عدّاد الاستخدام لشهر إصدار الشهادة** (يُشتقّ من `createdAt`، لا الشهر الجاري) بمعاملة
  ذرّية وبحدّ أدنى صفر (لا يصبح سالباً). الواجهة: زر `IconTrash` أحمر + نافذة
  `DeleteCertificateModal` تتطلّب **كتابة اسم المتدرّب** لتفعيل الحذف (حماية من الحذف
  العرضي)؛ صفحة التفاصيل تُعيد التوجيه للقائمة بعد الحذف. **قرار/مقايضة المالك:** نموذج
  الحصة = «إصدار شهري» (الإلغاء لا يُرجِع)، لكن الحذف يُرجِع الحصة بطلب صريح — فيفتح ثغرة
  إعادة تدوير داخل الشهر (إصدار→حذف→إصدار يتجاوز الحدّ)؛ مقبولة كقرار منتج.
- **تخطيط صفّ القائمة** (`dashboard/certificates`): وسم الحالة «نشطة/ملغاة» في **أقصى
  اليمين**، واسم المتدرّب في الوسط (`flex-1`)، ومجموعة أيقونات الإجراءات الموحّدة
  (تحميل/بريد/تحقق/إلغاء أو إعادة تفعيل/حذف) **يساراً** (زر الحذف مفصول بـ`ms-2`؛ وفي صفحة
  التفاصيل مدفوع لأقصى اليسار بـ`ms-auto`).
- **سجلّ نشاط الشهادة محدود (تخفيف القاعدة):** يُحتفَظ بـ**آخر 5 أحداث فقط لكل شهادة**
  (الأحدث زمنياً، أيّاً كان النوع)؛ عند إنشاء حدث جديد يُحذف الأقدم تلقائياً. المصدر الموحّد
  `services/certificateEvents.js` (`logCertificateEvent` = إنشاء + تقليم، و`pruneCertificateEvents`
  + ثابت `EVENT_RETENTION=5`). تمرّ به أحداث `opened`/`downloaded`/`shared`/`added_to_linkedin`
  (`verificationController`) و`emailed` (`certificateMailer`)؛ والإلغاء/إعادة التفعيل
  (`certificateController`) يبقيان الإنشاء المتداخل ذرّياً مع تغيير الحالة ثم يستدعيان
  `pruneCertificateEvents`. حدث `issued` (الأول) لا يحتاج تقليماً. `getCertificate` يجلب
  `take: EVENT_RETENTION`. الأحداث القديمة المتراكمة تُقلَّم تدريجياً مع أول حدث جديد لكل شهادة.

---

## حزمة المشاركة والانتشار (تحويل كل شهادة لأداة تسويق)

ثلاث ميزات تسويقية فورية حول صفحة التحقق العامة ولوحة المالك (قيمة تشغيلية في السوق العربي):
- **QR عام كصورة:** `GET /api/verify/:code/qr.png` (`controllers/qrController.js`، يُعيد استخدام
  مكتبة `qrcode` لإخراج PNG لرابط `verifyUrl(code)`؛ 404 إن لم توجد؛ تحت محدِّد `/api/verify`
  60/د؛ كاش يوم). تعرضه صفحة التحقق `<img>` بعنوان «امسح للتحقق الفوري» (لغير الملغاة).
  (الـQR موجود أصلاً داخل PDF؛ هذه نسخة ظاهرة على الصفحة + للمعاينات.)
- **روابط المشاركة (نقية + مختبَرة):** `certify-web/src/lib/share.ts` —
  `whatsappShareUrl({text,url,phone?})` (مع هاتف → `wa.me/<digits>`؛ بدونه → نافذة عامة؛
  تطبيع الهاتف لأرقام فقط)، `twitterShareUrl`، `shareText` (نصّ عربي يذكر المتدرّب/الدورة/الجهة).
  مغطّاة بـ`share.test.ts` (Vitest). أيقونات `IconWhatsapp`/`IconX`/`IconCopy` في `icons.tsx`.
- **صفحة التحقق = صفحة إنجاز قابلة للمشاركة:** `verify/[code]/page.tsx` — `generateMetadata`
  يضيف Open Graph + Twitter Card (عنوان «{المتدرّب} — {الدورة} | شهادة موثّقة»، صورة = شعار
  المنظمة إن وُجد) فتظهر معاينة غنية عند المشاركة؛ + صفّ مشاركة (واتساب/X/نسخ الرابط) في
  `CertificateActions` (client، يستدعي `track(shared)`؛ النسخ عبر `navigator.clipboard`).
  **قرار:** لا توليد صورة OG ديناميكية بالعربية (تعقيد خطوط satori) — يُكتفى بالشعار/الافتراضي.
- **واتساب في لوحة المالك:** زر واتساب في **صفحة تفاصيل الشهادة** يفتح `whatsappShareUrl`
  برابط التحقق + نصّ تهنئة، ويُمرّر `recipient_phone` (أُضيف إلى `presentCertificate`
  و`CertificateDetail`) للإرسال المباشر للمتدرّب إن توفّر هاتفه. لغير الملغاة. (أُزيلت أيقونة
  واتساب من صفّ قائمة الشهادات بطلب المالك — تبقى في صفحة التفاصيل وفي صفحة التحقق العامة.)
...
## محفظة المتدرّب (Trainee Certificate Wallet)

جزء موجّه **للمتدرّب** (مثل المنافسين): يجمع كل شهاداته الصادرة عبر المنصّة في مكان واحد، **بلا حساب**.
- **التدفّق:** صفحة `/wallet` العامة → المتدرّب يُدخل بريده → `POST /api/wallet/request` يُرسل **رابطاً
  سحرياً** لبريده إن وُجدت شهادات (ردّ **عام دائماً** `{ok:true}` — لا كشف وجود). الرابط
  `/wallet/<token>` → `GET /api/wallet/:token` يعرض كل شهادات ذلك البريد (غير الملغاة) عبر الأكاديميات:
  تحميل PDF + صفحة التحقق + مشاركة (واتساب/X).
- **الأمان:** التوكن **JWT موقّع** (`typ:"wallet"`, email, صلاحية 7 أيام، `HS256`) يُسلَّم فقط لذلك
  البريد — فحيازة الرابط تُثبت ملكية البريد. لا تعداد (الردّ موحّد). محدِّدات معدّل في `server.js`:
  `/api/wallet` 30/دقيقة + `POST /api/wallet/request` 5/ساعة/IP (كبح إغراق البريد). **بلا schema/جداول
  جديدة** (التوكن stateless، والاستعلام بالبريد على `Certificate.recipientEmail`).
- **الملفات:** `controllers/walletController.js` · `services/email/walletTemplates.js` (بريد ثنائي اللغة،
  يتبع تثبيت الإنجليزية) · الواجهة `lib/wallet.ts` + `app/wallet/page.tsx` (طلب) + `app/wallet/[token]/page.tsx`
  (عرض) + رابط فوتر «محفظة شهاداتي» + مفاتيح i18n `wallet.*`. اختبار `test/wallet.test.js` (لا تعداد،
  قراءة محميّة بالتوكن، رفض توكن غير صالح/نوع خاطئ).
