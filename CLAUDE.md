...
- **واتساب في لوحة المالك:** زر واتساب في صفّ قائمة الشهادات وصفحة التفاصيل يفتح
  `whatsappShareUrl` برابط التحقق + نصّ تهنئة، ويُمرّر `recipient_phone` (أُضيف إلى
  `presentCertificate` و`CertificateDetail`) للإرسال المباشر للمتدرّب إن توفّر هاتفه. لغير الملغاة.
