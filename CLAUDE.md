...
- **مصدر أسعار موحّد (الواجهة):** `certify-web/src/lib/pricing.ts` هو المصدر
  الوحيد لأسعار/أسماء الباقات المعروضة (يجب أن يطابق `prisma/seed.js`). الصفحات
  الثلاث `app/page.tsx` و`app/pricing/page.tsx` و`app/dashboard/billing/page.tsx`
  تستورد `PLAN_PRICING`؛ نصوص التسويق (المزايا/الوسوم) تبقى محليّة بكل صفحة.
  (كانت الأسعار مكرّرة ومختلفة — الهبوط أظهر $49/$99 خطأً.) الباقات تَعِد فقط
  بالمبنيّ فعلاً (أُزيل ذكر «أعضاء فريق» و«API» و«White-label/نطاق فرعي» الغامض).
