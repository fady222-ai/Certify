...
- **القالب الموجّه + لوحة «محتوى الشهادة»:** القالب 21 (الشعار أعلى → «شهادة» → جملة الأكاديمية →
  فوتر: QR وسط + التاريخ يميناً وتوقيع الأكاديمية يساراً) يضمّ عنصرَي نصّ موسومين `role:"title"`
  و`role:"body"`. لوحة **«محتوى الشهادة»** في المحرّر (`TemplateEditor.tsx`) **ظاهرة دائماً لأي قالب**:
  قائمة **نوع الشهادة** (`CERT_TYPES`) تكتشف العنوان (role أو أي نصّ يبدأ بـ«شهادة») فتجعله «شهادة
  حضور/تقدير/…» — وتُنشئ عنواناً إن لم يوجد؛ و**نصّ الشهادة** (`textarea`) لتعديل الجملة، مع زر
  «إضافة جملة الأكاديمية» إن لم توجد. تحكّم وقت التصميم يُحفظ في القالب. `canvas.ts`
  (`buildObject`/`objectToElement`) يحفظ `role` عبر دورة الحفظ (مغطّى بـ`canvas.test.ts`). بقية
  أدوات المحرّر (نص/متغيّر/مستطيل/QR/صورة/خلفية/الخصائص) دون تغيير.
