...
- **الواجهة:** البنية والمسارات تعمل في الخلفية (للتحقق الخارجي/LinkedIn/المحافظ)، لكن **لا
  زرّ مكشوف للمستخدم العادي** حالياً — أُخفي زر «الشهادة الرقمية» واللافتة لأن تنزيل ملف JWT
  خام يربك المتدرّب (قرار المالك). القدرة باقية في `CertificateActions` (prop `openBadge`،
  افتراضياً مُطفأ) لإعادة تفعيلها لاحقاً عبر صفحة عرض أنيقة بدل التنزيل الخام.
