...
- **وضع dev:** إن لم تُضبط مفاتيح بوابة، يتم التبديل المباشر للباقة بدون دفع — **في
  ...
  محدود 20/دقيقة/IP. الواجهة: `GatewayPicker` يسرد البوّابات الثلاث ويعرض غير المهيّأة
  **«غير مفعّلة»** معطّلة (بدل إخفائها)، و`resolveGatewayChoice` يفتح النافذة حتى عند
  صفر بوّابة ليرى المستخدم السبب. صفحة أدمن البوّابات تُحذّر إن فُعّلت بوّابة بحقول ناقصة.
